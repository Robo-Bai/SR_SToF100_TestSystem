# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：SR-SToF100-test-system -> SR_SToF100_system_V1.1
@IDE    ：PyCharm
@Author ：BJW
...
class DriveCAN(PCANBasic):
                                    ...
                                    if readResult[1].DATA[0] == 0x05:
                                        ...
                                        if distance != 0x96:
                                            ...
                                            if signal_strength > 0x1B58:#信号强度过滤 > 7000
                                                fill_distance_strength(Polar_angle[num], distance, signal_strength, global_value.Data_3D)
                                                ...
                                                mainWindow.DataDisplay_Window.textBrowser.append("样点编号：%s  取点详情：角度 = %.5f 测距 = %s cm 信号强度 = %s 3D层数：%s"
                                                                                      % (num, angle_data, distance, signal_strength, global_value.Data_3D))
                                            else:
    ...
    def can_X_ReadStatusword(self, chanel=PCAN_USBBUS1):
        # def thread_X_readStatus():
        #     while global_value.X_bcak_completedFlag == 0:
                ...
                if (readResult[0] & PCAN_ERROR_QRCVEMPTY) != PCAN_ERROR_QRCVEMPTY:
                    ...
                    if readResult[0] != PCAN_ERROR_QRCVEMPTY:
                        # 过滤返回的CAN报文，只取有数据的一组
                        if readResult[1].ID == 0x581:
                            if readResult[1].DATA[4] == 0x37:
                                if readResult[1].DATA[5] == 0xD6:
                                    global_value.X_bcak_completedFlag = 1
                                    print(global_value.X_bcak_completedFlag)
                                else:
                                    pass
                            else:
                                pass
                    ...
                    pass
        # thread_X_readstatus = Thread(target=thread_X_readStatus,
        #                       args=())
        # thread_X_readstatus.start()
...
#读取图片
path_image = projectPath + "\\Matplot_data\\Distance_image.png"
strength_image_path = projectPath + "\\Matplot_data\\Strenght_image.png"
...
###############################################################################################
        ...
        self.actionCSV_outliers.triggered.connect(self.OpenCSV_flie)
        #打开版本信息
    ...
    def OpenPDF_report_flie(self):
        ...
        PDF_directory    = projectPath + '\\PDFs\\'              #对话框默认路径
    ...
    def OpenPNG_flie(self):
        ...
        PNG_directory    = projectPath + '\\Matplot_data\\'              #对话框默认路径
    ...
    def OpenCSV_flie(self):
        ...
        CSV_directory    = projectPath + '\\Matplot_data\\'              #对话框默认路径
...
#################################################################################
    ...
    def X_Test_active(self):
        ...
        def thread_X_Test_active():
            ...
            time.sleep(0.05)
            DriveCAN().can_knicoX_write_manual(send_data=[0x23, 0x7A, 0x60, 0x00, 0x2D, 0xF9, 0xFF, 0xFF])   #1mm分辨率遍历，1747
            #DriveCAN().can_knicoX_write_manual(send_data=[0x23, 0x7A, 0x60, 0x00, 0xBC, 0xBB, 0xFF, 0xFF])   #10mm分辨率遍历，17476
            #设置梯形速度为50rpm
            time.sleep(0.05)
    ...
    def outliers_data(self):
        ...
        :return:
        '''
        rate_num = global_value.outliersRata_flag
        rate_data = ((global_value.PDF_outliers_num)/(global_value.PDF_normal_num))*100
        fill_outliers_rate(rate_num, rate_data)
        OutliersRate_image()
    ...
    def Test_Start_func(self):
        ...
        X_back_DemandFlag = 0   #X轴遍历一轮后发起返回初始位置的需求标志位
        ...
        X_TestRange = int((global_value.X_goal_location) - (global_value.DATA_distance))  #X轴的遍历区间
        Z_TestRange = int((global_value.Z_goal_location / 10) - (global_value.DATA_vertical / 10))  #Z轴的遍历区间
        print(X_TestRange)
        print(Z_TestRange)
        self.SToF_CAN_init()
        self.XZR_axle_enable()
        ...
        clear_fill_outliers()   #清空异常点率记录的数据
        time.sleep(0.5)

        def thread_work():
            # if global_value.stop_flag == 0:
                # num_distance = 0
                for vertical_num in range(Z_TestRange):
                    # 垂直遍历Z轴动作
                    if global_value.stop_flag == 0:
                        for num in range(X_TestRange):
                            if global_value.stop_flag == 0:
                                #清空值
                                global_value.normal_num = 0
                                global_value.outliers_num = 0
                                #水平遍历X轴、R轴动作
                                ...
                                if global_value.stop_flag == 0:
                                    ...
                                    Strength_Image()                  #绘制信号强度图
                                    time.sleep(0.5)
                                    self.show_strength_image()
                                    clear_fill_strength()             # 清空信号强度记录的数据
                                    time.sleep(1)

                                    self.X_Test_active()              #X轴移动

                                    time.sleep(0.5)
                                    global_value.DATA_distance = global_value.DATA_distance + 1
                                    global_value.PDF_normal_num = global_value.normal_num                     #正常点个数赋值给生成PDF值计算
                                    global_value.PDF_outliers_num = global_value.outliers_num                 #异常点个数赋值给生成PDF值计算
                                    global_value.outliersRata_flag = global_value.outliersRata_flag + 1       #PDF类异常点率的标志位
                                    print('正常点：%s   错误点：%s  %s'%(global_value.PDF_normal_num,global_value.PDF_outliers_num,global_value.outliersRata_flag))
                                    self.outliers_data()
                                else:
                                ...
                                break
                        X_back_DemandFlag = 1
                        global_value.X_bcak_completedFlag = 0
                    else:
                        pass
                        ...
                        if X_back_DemandFlag == 1:
                            ...
                            while global_value.X_bcak_completedFlag == 0:
                                DriveCAN().can_knicoX_write_manual(
                                    send_data=[0x40, 0x41, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00])  # 读取驱动器状态字，判断X轴是否已经回到初始指定位置
                                time.sleep(0.1)
                                print('正在检查是否以回初始位置')
                            ...
                            global_value.Data_3D = global_value.Data_3D + 1                      #3D数据层数+1
                            time.sleep(1)
                ...
                self.MainUi_textBrowser.append("检测已【完成】，请点击【生成PDF报告】输出检测结果")

        thread_work_main = Thread(target=thread_work,
                                  args=())
        thread_work_main.start()

    def X_TestRange_back(self):
            X_backBasicNum = 0x4444
            X_backMultipleNum = int((global_value.DATA_distance / 10) - (global_value.X_init_location / 10))
            # X_backMultipleNum = 2   #测试用
            print(global_value.DATA_distance)
            print(global_value.X_init_location)
            print(X_backMultipleNum)
            X_testRange_back_list = list(bytearray.fromhex((hex(X_backBasicNum * X_backMultipleNum)[2:]).zfill(8)))
            DriveCAN().can_knicoX_write_manual(send_data=[0x2F, 0x60, 0x60, 0x00, 0x01, 0x00, 0x00, 0x00])
            DriveCAN().can_knicoX_write_manual(send_data=[0x23, 0x7A, 0x60, 0x00,
                                                          X_testRange_back_list[3], X_testRange_back_list[2],
                                                          X_testRange_back_list[1], X_testRange_back_list[0]])
            time.sleep(0.05)
            DriveCAN().can_knicoX_write_manual(send_data=[0x23, 0x81, 0x60, 0x00, 0x40, 0xA7, 0x0D, 0x00])
            # 相对位置：0x4F->0x5F，X轴移动一次
            time.sleep(0.05)
            DriveCAN().can_knicoX_write_manual(send_data=[0x2B, 0x40, 0x60, 0x00, 0x4F, 0x00, 0x00, 0x00])
            time.sleep(0.05)
            DriveCAN().can_knicoX_write_manual(send_data=[0x2B, 0x40, 0x60, 0x00, 0x5F, 0x00, 0x00, 0x00])
...
pdfmetrics.registerFont(TTFont('pingbold', 'msyh.ttc'))
...
class PDFGenerator:
    def __init__(self, filename):
        ...
        self.file_path = (projectPath + '\\PDFs\\PDF')
    ...
    def genTaskPDF(self, home_data, task_data):
        ...
        img = Image(projectPath + '\\Icon\\sr.jpg')
        ...
        story.append(Spacer(1, 3 * mm))
        img_uart_path = projectPath + '\\Matplot_data\\Distance_image.png'
        img_uart = reportImage(img_uart_path)
        img_uart.drawHeight = 110 * mm
        img_uart.drawWidth = 120 * mm
        ...
        #插入异常率图片
        story.append(Paragraph("SToF100异常率结果图", self.table_title_style))
        story.append(Spacer(1, 3 * mm))
        img_uart_path = projectPath + '\\Matplot_data\\OutliersRate_image.png'
        img_uart = reportImage(img_uart_path)
        img_uart.drawHeight = 100 * mm
        img_uart.drawWidth = 110 * mm
        img_uart.hAlign = TA_LEFT
        story.append(img_uart)
...
class Location_Window(QMainWindow, Ui_location):
    ...
    def __init__(self):
        ...
        self.X_confirm_Button.clicked.connect(self.X_goal_confirm)
        #Z轴位置确定
        self.Z_confrim_Button.clicked.connect(self.Z_location_confirm)
        self.Z_confrim_Button.clicked.connect(self.Z_goal_confirm)
    ...
    def X_location_confirm(self):
        ...
        if self.X_spinBox.value() == 50:
            DriveCAN().can_knicoX_write_manual(send_data=[0x23, 0x7A, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00])
    ...
    def X_goal_confirm(self):
        '''
        X 轴位置确认函数：确认X轴检测末尾区间
        :return:
        '''
        global_value.X_goal_location = self.X_goal_spinBox.value()

    def Z_goal_confirm(self):
        '''
        Z轴位置确认函数：确认Z轴检测末尾区间
        :return:
        '''
        global_value.Z_goal_location = self.Z_goal_spinBox.value()
    ...
    def Z_location_confirm(self):
        ...
        if self.Z_spinBox.value() == 0:
            DriveCAN().can_knicoZ_write_manual(send_data=[0x23, 0x7A, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00])
            global_value.Z_init_location = 0
            self.location_textBrowser.append("Z轴位置当前在原点")
        elif self.Z_spinBox.value() == 50:  # 读取spinBox控件的数值
