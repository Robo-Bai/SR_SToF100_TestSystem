...
#include "..\components\drivers\crypto\sr_md5.h"
...
void Md5Update(Md5Context* Context,  void const*  Buffer,  uint32_t BufferSize)
{
    ...
    if( used )
    {
        ...
        if( BufferSize < free )
        {
            secure_memcpy( &Context->buffer[used], Buffer, BufferSize );
            return;
        }

        secure_memcpy(&Context->buffer[used], Buffer, free );
    ...
    secure_memcpy( Context->buffer, Buffer, BufferSize );
}
...
void Md5Finalise(Md5Context* Context, MD5_HASH*  Digest)
{
    ...
    if(free < 8)
    {
        secure_memset( &Context->buffer[used], 0, free );
        TransformFunction( Context, Context->buffer, 64 );
        used = 0;
        free = 64;
    }

    secure_memset( &Context->buffer[used], 0, free - 8 );
