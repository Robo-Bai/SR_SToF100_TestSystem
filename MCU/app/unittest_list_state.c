...
#include "..\app\include\ev_process.h"
 ...
 // 读取状态寄存器
static uint16_t state_msg[][2] = {

	{0x2000, 18},  /* 系统信息 aa 06 07 10 00 20 11 00 26 55  */
	{0x2011, 16}, /* 系统通信状态 */
	{0x2400, 8 }, /* 运动控制执行任务状态 */
	{0x2408, 17}, /* 当前运动状态 */
...
static int32_t state_handler_callback(void *ops, void *m, uint16_t len)
{
...
static int32_t utest_state_init(struct unittest_operations *uops)
{
	// TODO
	uops->ut_map->handler_callback = state_handler_callback;
...
static int32_t utest_state_dispatch( struct unittest_operations *uops)
{
	msg_event msg;
	...
	if (uops->ut_map->real_count_tx < uops->ut_map->target_count){
		...
		msg.len = offsetof(struct reg_map, msgto)/sizeof(uint8_t);
		...
		memcpy(&uops->ut_map->event.regs->count, 
			(uint8_t *)((uint8_t *)uops->ut_map->event.regs->msgto+2+cur_ietms*uops->ut_map->event.offset), 
...
static struct unittest_operations state_ops = {
	.name			= "state",
	.ut_map			= ut_list_state,
	.init			= utest_state_init,
	.dispatch		= utest_state_dispatch,
};
...
int32_t utest_register_state(void)
{
	...
	return utest_register_ops(&state_ops);
}
