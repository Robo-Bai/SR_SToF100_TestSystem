...
#include "..\app\include\ev_process.h"
 ...
 // CMD 占用8个寄存器 = 8*4
static uint8_t cmd_msg[][8*4] = {

	{	/* 与SROS建立连接, 传输完整帧总计bit = 42B*10 = 420bit,传输时间：1/460800*420 = 0.911ms  */
		0x21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, \
...
//	},
	...
	{	/* 启动子任务 */
		0x24, 0x00, 0x00, 0x00, 0xa1, 0x00, 0x00, 0x00, 0xf0, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x0b, 0x00, \
		0x00, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x0d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 
	},
...
static struct utest_map ut_list[] = {
	{
		.event	= {
            .type 		= EV_CMD,
			.items		= sizeof(cmd_msg)/sizeof(cmd_msg[0]),
            .offset		= sizeof(cmd_msg[0]),
            .regs		= reg_msg,
        },
...
static int32_t cmd_handler_callback(void *ops, void *m, uint16_t len)
{
...
//			//sr_data_print("msg", (uint8_t *)m, len);
//		}
		tx_flag = true;
	}
...
static int32_t utest_cmd_init(struct unittest_operations *uops)
{
	// TODO
	
	uops->ut_map->handler_callback = cmd_handler_callback;
...
static int32_t utest_cmd_dispatch( struct unittest_operations *uops)
{
	msg_event msg;
	static uint16_t cur_ietms = 0;	// 当前测试项
	
	//sr_kprintf("%s, ev->type=0x%02x, ev->items=%d\r\n", __FUNCTION__, uops->ut_map->event.type, uops->ut_map->event.items);
	if (uops->ut_map->real_count_tx < uops->ut_map->target_count){
		...
		if(get_delta_time(uops->ut_map->last_timestamp, uops->ut_map->cur_timestamp) > uops->ut_map->timeout){
			...
			sr_kprintf("error, %s timeout %d ms\r\n", __FUNCTION__, get_delta_time(uops->ut_map->last_timestamp, uops->ut_map->cur_timestamp));
			return -1;
		}

		memset(&msg, 0, sizeof(msg));
		msg.type = uops->ut_map->event.type;
		msg.len = uops->ut_map->event.offset + offsetof(struct reg_map, msgto)/sizeof(uint8_t);
		...
		//sr_data_print("ev->msgto", (uint8_t *)((uint8_t *)uops->ut_map->event.regs->msgto+cur_ietms*uops->ut_map->event.offset), \
			uops->ut_map->event.offset);
		cur_ietms += 1;
...
static struct unittest_operations cmd_ops = {
	.name			= "cmd",
	.ut_map			= ut_list,
	.init			= utest_cmd_init,
	.dispatch		= utest_cmd_dispatch,
};
...
int32_t utest_register_cmd(void)
{
	...
	return utest_register_ops(&cmd_ops);
}
