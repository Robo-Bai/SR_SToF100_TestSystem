# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：SR-SToF100-test-system -> GUI_0603_outliers
@IDE    ：PyCharm
@Author ：BJW
@Date   ：2021/6/3 10:48
=================================================='''
                                    ...
                                    if readResult[1].DATA[0] == 0x05:
                                        ...
                                        if distance != 0x96:
                                            ...
                                            if signal_strength > 0x1B58:#信号强度过滤 > 7000
                                                ...
                                                # textBrowser.MainUi_textBrowser.append("取点详情：极角 = %s 测距 = [%s] 信号强度 = %s"
                                                #                                       % (Polar_angle[num], distance, signal_strength))
                                                # textBrowser.MainUi_textBrowser.append("46")
                                            else:
                                                # fill_distance_strength(Polar_angle[num], distance, signal_strength, 1)
                                                # print("异常点已记录")
                                                pass
                                        else:
                                            num = num + 1
...
class Main_Window(QMainWindow, Ui_MainGUI):
    ...
    def __init__(self, parent=None):
        ...
        #【子窗口--生成PDF文件】在父窗口中的定义
        self.Label_Window = Label_Window()
        #【子窗口--Version】在父窗口中的定义
...
###############################################################################################
        #电机调试
        self.menu_motor.triggered.connect(self.OpenMotorControl)
        #打开最新生成的PDF报告
...
class PDFGenerator:
    ...
    def genTaskPDF(self, home_data, task_data):
        ...
        task_table = Table(task_data, colWidths=[65 * mm, 130 * mm], rowHeights=12 * mm, style=self.common_style)
...
class PDF_result():
    ...
    def pdf_result():
        ...
        """主程序"""
        #获得的产品数据
        home_data = {'report_code': global_value.ToF_type,
                   'task_name':'SToF100',
                   'report_date': data ,
                   'report_creator': global_value.ToF_person
                  }
        ...
        #PDF类，读取产品数据并生成PDF
        test = PDFGenerator("report_" + global_value.ToF_type)
...
class Label_Window(QMainWindow, Ui_Label):
    ...
    def OK_message(self):
        ...
        global_value.ToF_person = self.lineEdit_person.text()
