# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：SR-SToF100-test-system -> GUI_PCAN_work0524
@IDE    ：PyCharm
@Author ：BJW
@Date   ：2021/5/24 14:11
=================================================='''
...
class DriveCAN(PCANBasic):
    ...
    def can_read_SH200(self, chanel=PCAN_USBBUS1):  # 默认参数：通道
        ...
        def thread_SH200_DATA_read():
            ...
            Polar_angle = np.linspace((2*np.pi)/7.2, (2*np.pi)/2.769, 30, endpoint=True)
                                ...
                                if readResult[1].ID == 0x511:
                                    if readResult[1].DATA[0] == 0x04:
                                        if readResult[1].DATA[1] != 0x96:
                                            fillOperation(Polar_angle[i], readResult[1].DATA[1])
                                            print("i = %s  id = %s Distance = [%s]"
                                                    % (i, Polar_angle[i],
                                                    readResult[1].DATA[1]))
                                        else:
                                            fillOperation(Polar_angle[i],0)
                                            print("i = %s  id = %s Distance = [%s]"
                                                  % (i,Polar_angle[i],0))
                                    else:
                                        pass
...
#################################################################################
    ...
    def Test_Start_func(self):
        ...
        # thread_init_main = Thread(target=thread_init,
        #                           args=())
        # thread_init_main.start()
        # thread_init_main.join()
        # time.sleep(5)
        # if R_init_flag == 1:
        def thread_work():
            # global SH200_work_flag
