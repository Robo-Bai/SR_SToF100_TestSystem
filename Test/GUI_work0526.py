# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：SR-SToF100-test-system -> GUI_work0526
@IDE    ：PyCharm
@Author ：BJW
...
class DriveCAN(PCANBasic):
    ...
    def can_read_SH200(self, chanel=PCAN_USBBUS1):  # 默认参数：通道
        ...
        def thread_SH200_DATA_read():
            ...
            Polar_angle = np.linspace((2*np.pi)/7.2, (2*np.pi)/2.769, 200, endpoint=True)
            num = 0
                        ...
                        # 确保接受队列可以接受新的信息
                            ...
                            if readResult[0] != PCAN_ERROR_QRCVEMPTY:
                                ...
                                if readResult[1].ID == 0x511:
                                    if readResult[1].DATA[0] == 0x04:
                                        if readResult[1].DATA[1] != 0x96:
                                            num = num + 1
                                            fillOperation(Polar_angle[num], readResult[1].DATA[1])
                                            print("num = %s  极角 = %s Distance = [%s]"
                                                    % (num, Polar_angle[num],
                                                    readResult[1].DATA[1]))
                                        else:
                                            num = num + 1
...
class Main_Window(QMainWindow, Ui_MainGUI):
    ...
    def __init__(self, parent=None):
        ...
        #打开图片
        self.PDF_Button.clicked.connect(self.show_image)
...
#################################################################################
    ...
    # # def show_image(self):
    #     # with open('CAN-DATA_0526.png', 'rb') as f:
    #     #     img = f.read()
    #     self.label = QtWidgets.QLabel(self.groupBox)
    #     self.label.setGeometry(QtCore.QRect(20, 20, 570, 520))
    #     self.label.setText("")
    #     self.label.setPixmap(QtGui.QPixmap("../CAN-DATA_0526.png"))
    ...
    #     self.label.setObjectName("label")
        ...
        # lab1 = self.QLable()
        # lab1.setPixmap(QPixmap('CAN-DATA_0526.png'))
